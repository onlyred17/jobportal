...
if ($stmt->rowCount() == 0) {
    ...
    header('Location: ../views/view_admin_forgot_password.php');
    exit;
}

// Fetch the admin member's full name (or any other necessary info)
$row = $stmt->fetch(PDO::FETCH_ASSOC);
$full_name = $row['full_name']; // Assuming 'full_name' is a column in your admin table

// Generate a random verification code
$verification_code = rand(100000, 999999);

// Update the verification code in the database for the admin member
$updateQuery = "UPDATE admin SET verification_code = :verification_code WHERE email = :email";
...
try {
    ...
    header('Location: ../views/view_admin_verify_code.php');
    exit;
