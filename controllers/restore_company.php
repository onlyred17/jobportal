...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT name FROM company WHERE id = :id AND is_deleted = 1";
    ...
    if ($company) {
        ...
        $query = "UPDATE company SET is_deleted = 0, deleted_at = NOW() WHERE id = :id";
        $stmt = $conn->prepare($query);
        $stmt->bindParam(':id', $id);
        ...
        if ($stmt->execute()) {
            $_SESSION['message'] = [
                'type' => 'success',
                'text' => "Company '{$companyName}' restored successfully!"
            ];
            ...
            $description = "Company '{$companyName}' restored"; // Description of the action
            ...
            $_SESSION['message'] = [
                'type' => 'danger',
                'text' => "Error restoring company '{$companyName}'."
            ];
        }
    } else {
        $_SESSION['message'] = [
    ...
    header('Location: ../views/view_staff_company_archieved.php');
    exit();
} 
